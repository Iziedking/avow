...
const API = process.env.AGENT_API ?? "http://localhost:8787";
...
async function main() {
  ...
  const out: any = await post("/agent", { mandateId: claimed.mandateId, instruction: process.env.INSTR ?? "swap 1 SUI to stablecoin" });
  if (out.error) return console.log("agent error:", out.error);
  console.log("REPLY:", out.reply); console.log("steps:", JSON.stringify(out.steps), "url:", out.swapUrl, out.error?("ERR:"+out.error):"");
